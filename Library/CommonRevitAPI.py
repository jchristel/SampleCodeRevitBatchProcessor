...
def setParameterValue(para, valueAsString, doc):
    ...
    if(para.StorageType == StorageType.ElementId):
        ...
        def action():
            ...
            try:
                para.Set(newId)
                actionReturnValue.message = 'Changed parameter value of type ID.' + oldValue + ' to: ' + valueAsString
            except Exception as e:
        ...
        def action():
            ...
            try:
                ...
                actionReturnValue.message = 'Changed parameter value of type double.' + oldValue + ' to: ' + valueAsString
            except Exception as e:
                actionReturnValue.UpdateSep(False, 'Failed with exception: ' + str(e))
            return actionReturnValue
        transaction = Transaction(doc,transactionName)
        ...
        def action():
            actionReturnValue = res.Result()
            try:
                para.SetV(int(valueAsString))
                actionReturnValue.message = 'Changed parameter value of type integer.'  + oldValue + ' to: ' + valueAsString
            except Exception as e:
                actionReturnValue.UpdateSep(False, 'Failed with exception: ' + str(e))
            return actionReturnValue
        transaction = Transaction(doc,transactionName)
        ...
        def action():
            ...
            try:
                ...
                actionReturnValue.message = 'Changed parameter value of type string: ' + oldValue + ' to: ' + valueAsString
            except Exception as e:
                actionReturnValue.UpdateSep(False, 'Failed with exception: ' + str(e))
            return actionReturnValue
        transaction = Transaction(doc,transactionName)
...
def DeleteViews(doc, viewRules, collectorViews):
    ...
    for v in collectorViews:
        ...
        if(util.EncodeAscii(v.Name)[0] != '<' and v.ViewType != ViewType.Internal and v.ViewType != ViewType.Undefined and v.ViewType != ViewType.ProjectBrowser and v.ViewType != ViewType.DrawingSheet and v.ViewType != ViewType.SystemBrowser):
            viewCounter =+ 1
...
def DeleteAllSheetsInModel(doc):
    ...
    for v in collectorSheets:
        if(v.ViewType == ViewType.DrawingSheet):
           ids.append(v.Id)
    if (len(ids)>0):
        returnvalue = DeleteByElementIds(doc,ids, 'deleting all sheets', 'sheets')
    else:
