...
set "_settingsStepOneFileName=BatchRvt.2022.ModifyChangeFamilyCategoryOne%%j.ALL.Settings.json"
...
%stamp% & echo.
%stamp% & echo ********************************* Modify Families - change family category ******************************************
%stamp% & echo.
...
if %_counter% lss %_len% (
  for /f "usebackq delims== tokens=2" %%j in (`set _obj[%_counter%]`) do (
    ...
    start "TaskModifyFiles%_counter%" 9>"%_lock%%_counter%" %_targetPath% --settings_file "%_settingsDiretoryPath%%_settingsStepOneFileName%"
    %stamp% & echo started Revit Batch Processor...Modifying Family Files change family category... %_taskCounterDisplay% of %_len%
    %stamp% & echo [settings file used: %_settingsStepOneFileName%]
  )
...
:: Finish up
%stamp% & echo Modifying Files : Done
%stamp% & echo.
