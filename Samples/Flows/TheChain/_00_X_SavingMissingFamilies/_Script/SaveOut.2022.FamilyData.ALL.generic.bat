...
set "_cleanUpScriptPath=%_rootFolderPath%_Script\Post_SaveOutMissingData.py"

:: root directory where each session saves families in a per session sub-directory to
set _familyOutDirectory="%_rootFolderPath%_Users\%USERNAME%\_FamilyOut\*"
:: root directory to where combine saved out families to
set _combinedFamilyOutDirectory="%_rootFolderPath%_Users\%USERNAME%\_Output\combined"
...
if %toggleDebug%==1 (
  ...
  %stamp% & echo _familyOutDirectory: %_familyOutDirectory%
  %stamp% & echo _combinedFamilyOutDirectory: %_combinedFamilyOutDirectory%
...
if %_counter% lss %_len% (
  for /f "usebackq delims== tokens=2" %%j in (`set _obj[%_counter%]`) do (
    ...
    %stamp% & echo started Revit Batch Processor....Saving Missing Families ... %_taskCounterDisplay% of %_len%
...
%stamp% & echo.
%stamp% & echo ******************** combine written out families into one directory ****************
%stamp% & echo.
%stamp% & echo from: %_familyOutDirectory%
%stamp% & echo to: %_combinedFamilyOutDirectory%
%stamp% & echo.

for /D %%f in (%_familyOutDirectory%) do (
  xcopy "%%f" %_combinedFamilyOutDirectory% /I /s /E /Y
)

%stamp% & echo.
%stamp% & echo ******************** clean up working directory ****************
%stamp% & echo.
%stamp% & echo cleaning up: %_familyOutDirectory%
...
:: Finish up
%stamp% & echo.
%stamp% & echo Saving missing families : Done
%stamp% & echo.
