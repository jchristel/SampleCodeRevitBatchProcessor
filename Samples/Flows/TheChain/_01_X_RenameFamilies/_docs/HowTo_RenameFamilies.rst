...
What makes the renaming of families complicated is the fact that families regularly contain other families nested into them. When renaming a family on a file server, these nested families do not know about the name change
...
This flow will, as a pre- process, rename Revit family files and any catalogue files associated as per rename directives in the library folder in a drive location. It will then create a single task list file which will contain all the host family file path
...
Families will be saved to a temp location. Once all families have been processed the changed families will be copied back to the original family location thereby overwriting the existing files.
