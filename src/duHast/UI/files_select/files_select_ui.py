...
def main(argv):
    ...
    if got_args:
        ...
        if len(revit_files) > 0:
            ...
            print(
                "Source File Extension {}".format(xaml.Root.DataContext.source_file_extension)
            )
            print("Include sub dirs {}\n".format(xaml.Root.DataContext.include_sub_dirs))
            ...
            if ui_result:
                ...
                buckets = wl.distribute_workload(
                    settings.output_file_num, view_model.selected_files, fl.get_file_size
                )
            ...
            # show message box
            output_with_time_stamp("No files found matching extension: {}!".format(settings.revit_file_extension))
            sys.exit(2)
    else:
