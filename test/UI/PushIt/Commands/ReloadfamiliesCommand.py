...
class ReloadFamiliesCommand(CommandBase):
    ...
    def __init__(self, revit_model, families_selection_view_model, family_selection_view_navigation_service, execute=None):
        ...
        self.family_selection_view_navigation_service = family_selection_view_navigation_service
        self._execute = execute
        
        #sub scribe to property change event to enable or disable submit button
        self.families_selection_view_model.add_PropertyChanged(self.OnViewModelPropertyChanged)
    ...
    def CanExecute(self, parameter):
        ...
        return (self.families_selection_view_model.LibraryPath is not None and 
                directory_exists(self.families_selection_view_model.LibraryPath))
    
    def Execute(self, parameter):
        print("In execute")
        families_in_model =  self.revit_model.get_all_families()
        
        # TODO: store selected familis in revit model property
        for fam in self.families_selection_view_model.Families:
            if fam.IsSelected:
                # find the family in the revit family model
                # Finding the object
                result = [obj for obj in families_in_model if obj.family_name == fam.FamilyName and obj.family_category == fam.FamilyCategory]
                if(result):
