...
from duHast.Revit.Views.Reporting.views_report import (
    get_views_report_data,
    get_views_report_data_filtered,
    write_views_data,
    write_view_data_by_property_names,
)
...
def test_get_views_report_data(doc):
    """
    ...
    message = "-"
    try:
        result = get_views_report_data(doc, REVIT_TEST_FILE_NAME)
        expected_result = [
            [
...
def test_get_views_report_data_filtered(doc):
    """
...
def test_write_views_data(doc):
    ...
    try:
        ...
        result = write_views_data(
            doc, os.path.join(tmp_dir, "views_test.txt"), REVIT_TEST_FILE_NAME
        )
        print(result)
        ...
        message = (
            message
            + "\n"
            + ("An exception occurred in function test_write_views_data {}".format(e))
        )
        flag = False
    return flag, message


def test_write_view_data_by_property_names(doc):
    '''
    ...
    try:
        ...
        result = write_view_data_by_property_names(
            doc,
            os.path.join(tmp_dir, "views_test.txt"),
            REVIT_TEST_FILE_NAME,
            VIEW_DATA_FILTERS,
        )
        print(result)
...
def run_tests(doc, output):
    ...
    tests_common = [
        ["test_get_views_report_data", test_get_views_report_data],
        ["test_get_views_report_data_filtered", test_get_views_report_data_filtered],
        ["test_write_views_data", test_write_views_data],
        [
