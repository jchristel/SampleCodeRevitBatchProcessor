...
XAML_BY_VIEW ={
    FamiliesSelectionViewModel: os.path.join(settings.SCRIPT_DIRECTORY,  r"Views\FamiliesSelectionView.xaml"),
}
    ...
    with forms.ProgressBar(title='Reloading families: {value} of {max_value}', cancellable=True) as pb:
        try:
            for fam in families:
                ...
                reload_result = reload_family(doc=doc, family=revit_family, family_file_path=fam.family_file_path)
...
def reloaded_families_entry(doc, output, forms, families = None):
    """
    ...
    my_window = Reloader(
        xaml_path= os.path.join(settings.DUHAST_DIRECTORY,  r"UI\Objects\WPF\Views\MainWindow_py.xaml"),
    ...
    if(doc):
        if(len(REVIT_MODEL.families_to_reload)>0):
            reloader_result = reload_families (doc=doc, families= REVIT_MODEL.families_to_reload, forms=forms)
