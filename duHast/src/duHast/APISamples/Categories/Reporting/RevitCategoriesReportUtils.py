...
def build_report_data_by_category(doc, dic, family_cat, main_cat_name, doc_file_path):
    ...
    data = []
    for key in dic:
        row = [str(doc_file_path).encode('utf-8'),
        ...
        row.append(str(dic_line_weights[PROPERTY_LINE_WEIGHT_PROJECTION_NAME]).encode('utf-8'))
...
def get_report_data(doc, revit_file_path):
    ...
    sub_cats = get_main_sub_categories(doc) # i/e family is specialty equipment and all its associated sub categories
    family_cat = get_family_category(doc) # any 3D element which is set to 'None' in subcategory (if family is specialty equipment so is this element)
    other_cats = get_other_sub_categories(doc) # Imports in Families cats are here
    family_cat_name = list(family_cat.keys())[0]
    # build output
    data = build_report_data_by_category(doc, family_cat, family_cat_name, family_cat_name, revit_file_path)
    data = data + build_report_data_by_category(doc, sub_cats, family_cat_name, family_cat_name, revit_file_path)
    # check for imports
    if ('Imports in Families' in other_cats):
        data = data + build_report_data_by_category(doc, other_cats['Imports in Families'], family_cat_name, 'Imports in Families', revit_file_path)
    return data