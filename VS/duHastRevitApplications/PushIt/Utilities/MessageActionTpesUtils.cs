...
namespace duHastNet.PushIt.Utilities
{
    public class MessageActionTypesUtils
    {
        ...
        public static Utils.WPF.Stores.MessageTypes CombineMessageActionType(List<Utils.WPF.Stores.MessageTypes> messageTypes)
        {
            ...
            int logCount = messageTypes.Count(x => x == Utils.WPF.Stores.MessageTypes.Warning);
            ...
            // if there are no errors or info messages but warning messages, return warnings
            else if (logCount > 0)
            {
                return Utils.WPF.Stores.MessageTypes.Warning;
            }
