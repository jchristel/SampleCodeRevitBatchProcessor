...
Opens the **AtTheLibrary** WPF application â€” an interactive library browser that lets you view, filter, load, and edit families from a Schedule of Accommodation (SoA) data source without leaving Revit.
...
Use this button when you need to browse a family library, check what is already loaded in a project, or load and edit specific families in an organised, data-driven way â€” rather than using the Revit Load Family dialog.

## Notes

- The SoA CSV file path is configured within the AtTheLibrary window (browse or type the path, then click Load Data).
- See the [AtTheLibrary user guide](../../revit_apps/athelibrary/01_main_ui.md) for full documentation of the interface.
