...
Tools for managing Revit schedules â€” column widths, sheet placement overlaps, and room number filter values.
...
**Use when:** You want to snapshot which room number each schedule is currently filtered to â€” for example, before bulk-updating schedules or as part of a handover audit.
...
- You provide a prefix string (e.g. `"Room "`) â€” the text immediately after the prefix in the schedule name is used as the room number.
...
**Use when:** You need to restore or bulk-apply room filter values to schedules â€” for example, after a model was detached and schedules lost their filter settings, or when applying a filter snapshot from another model.
