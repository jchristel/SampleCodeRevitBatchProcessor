...
set "_settingsStepOneFileName=BatchRvt.2022.ModifyReloadFamiliesOne%%j.ALL.Settings.json"
:: file path to clean up script run at the very end
set "_cleanUpScriptPath=%_rootFolderPath%_Script\Post_ModifyLibraryFamilyReload.py"
...
if %_counter% lss %_len% (
  for /f "usebackq delims== tokens=2" %%j in (`set _obj[%_counter%]`) do (
    ...
    start "TaskReloadFams%_counter%" 9>"%_lock%%_counter%" %_targetPath% --settings_file "%_settingsDiretoryPath%%_settingsStepOneFileName%"
    echo started Revit Batch Processor...Reloading Family Files... %_taskCounterDisplay% of %_len%
